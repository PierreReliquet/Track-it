...
public class LoansDAO extends DBHelper {
    ...
    public List<Loan> getAllNonFinishedLoans() {
        return getAllNonFinishedLoans(null);
    }
    ...
    public int getNumberOfNonTerminatedLoans() {
        Cursor cursor = this.getReadableDatabase().query(
                LOANS_TABLE,
                new String[]{LOANS_COLUMN_ID},
                LOANS_COLUMN_END_DATE + " = 0", null,
                null, null, LOANS_COLUMN_ID);
        int amount = cursor.getCount();
        cursor.close();
        this.getReadableDatabase().close();
    ...
    public List<Loan> getLoanByProductId(String id) {
        Cursor cursor = this.getReadableDatabase().query(
                LOANS_TABLE,
                null,
                LOANS_COLUMN_BARCODE + " = ? AND " + LOANS_COLUMN_END_DATE
                        + "= 0", new String[]{id}, null, null,
                LOANS_COLUMN_ID);

        List<Loan> tmp = new ArrayList<Loan>();
        while (cursor.moveToNext()) {
            tmp.add(createLoanFromCursor(cursor));
        }
        return tmp;
    }
}
